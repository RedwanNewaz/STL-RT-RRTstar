...
class RRTStar():
    ...
    class Node:
        ...
        def __eq__(self, other):
            ...
            # Check if both objects have the required attributes
...
def plot_human_referential(followed_path_human_referential):
    ...
    plot_circle(0, 0, 20)
    plt.plot([x for (x, y) in followed_path_human_referential], [y for (x, y) in followed_path_human_referential],
             'r--')
    plt.grid(True)
    # plt.show()
...
def main():
    ...
    test_several_zones = Conjunction([Always(STLPredicate2D(0, 1, 400, 450, 50, 100), 15, 25),
                                      Always(STLPredicate2D(0, 1, 100, 150, 350, 400), 35, 55)])
    ...
    move_pepper = [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2, 2.2, 2.4, 2.6, 2.8, 3, 3.2, 3.4, 3.6, 3.8, 4, 4.2, 4.4,
